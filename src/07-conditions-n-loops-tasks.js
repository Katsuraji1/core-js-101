...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < ccnArray.length; i += 1) {
    let num = Number(ccnArray[i]);
...
function getDigitalRoot(num) {
  const sum = num.toString().split('').map((el) => Number(el)).reduce((prev, curr) => prev + curr);
...
function getCommonDirectoryPath(pathes) {
  const chars = pathes.map((el) => el.split('/'));
  let result = '';
  for (let i = 1; i < chars.length; i += 1) {
    for (let j = 0; j < chars[0].length; j += 1) {
      if (chars[0][j] === chars[1][j]) {
        if (chars[1][j] === chars[chars.length - 1][j]) {
          result += `${chars[0][j]}/`;
        }
      }
    }
